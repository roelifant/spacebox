            ...
            <div class="w-full border-2 p-4 flex justify-between">
                <div class="w-3/12 relative">
                    <div class="absolute transition-opacity duration-300" :class="{ 'opacity-0': !itemInfo }">
                        <h2 class="text-lg font-bold">{{ itemInfoName }} <span class="text-gray-500 font-normal text-sm">({{
                            itemInfoType }})</span> </h2>
                ...
                <div class="w-9/12 pl-12">
                    ...
                    <div class="grid grid-cols-3 xl:grid-cols-4 w-full py-2 gap-2">
                        ...
                        <div @mouseenter="showFuelInfo()" @mouseleave="hideItemInfo()"
                            ...
                            <button @click="buyFuel($event)" :disabled="!canBuyFuel" class="
                                            border-2
                                            text-white
                        ...
                        <div @mouseenter="showAmmoInfo()" @mouseleave="hideItemInfo()"
                            class="border-2 flex items-center w-full gap-2">
                            ...
                            <button @click="buyAmmo($event)" :disabled="!canBuyAmmo" class="
                                            border-2
                                            text-white
                                            uppercase text-xs font-bold
                                            px-2 py-1 transition-colors
                        ...
                        <div @mouseenter="showRepairInfo()" @mouseleave="hideItemInfo()" v-show="needsRepair"
                            class="border-2 flex items-center w-full gap-2">
                            ...
                            <button @click="buyRepair($event)" :disabled="!canBuyRepair" class="
                                            border-2
                                            text-white
                                            uppercase text-xs font-bold
                                            px-2 py-1 transition-colors
                    ...
                    <div class="grid grid-cols-3 xl:grid-cols-4 w-full py-2 gap-2">
                        ...
                        <div class="border-2 flex items-center gap-2 w-full" @mouseenter="showCargoInfo(cargo.key)"
                            @mouseleave="hideItemInfo()" v-for="cargo in selling" :key="cargo.key">
                            <div class="w-14 h-14 flex justify-center items-center border-r-2">
                                <img :src="'./img/icons/' + title(cargo.key) + '.png'" :alt="cargo.key"
                                    class="w-10 h-10 object-contain">
                            </div>

                            <div class="flex flex-col h-full justify-center pl-2 overflow-hidden flex-grow">
                                <p class="truncate text-sm"><span class="font-bold">{{ title(cargo.key) }}</span>
                                    ({{ cargo.count }})</p>
                                <p class="text-xs">§ {{ Market.getBuyingPrice(cargo.key) }}</p>
                            </div>

                            <button @click="buyCargo($event, cargo.key)" :disabled="!Market.canBuy(cargo.key)" class="
                                            border-2
                                            text-white
                                            uppercase text-xs font-bold
                                            px-2 py-1 transition-colors
                    ...
                    <div class="grid grid-cols-3 xl:grid-cols-4 w-full py-2 gap-2">
                        <div class="border-2 flex items-center gap-2 w-full"
                            @mouseenter="showItemInfo(upgrade.name, upgrade.description, 'upgrade')"
                            @mouseleave="hideItemInfo()" v-for="upgrade in upgrades.slice(0, 4)" :key="upgrade.key">
                            ...
                            <button @click="upgrade.purchase()" :disabled="!upgrade.canBuy()" class="
                                            border-2
                                            text-white
                                            uppercase text-xs font-bold
                                            px-2 py-1 transition-colors
